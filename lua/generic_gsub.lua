--[[
  This module defines the `generic_gsub' function which provides
  string.gsub-compatible interface. It allows to plug in various
  search/match engines (such as PCRE or POSIX regex library).

  Shmuel Zeigerman; Jun 2004 - Nov 2006.
--]]
...
--   which must correspond to the library determined by argument 'new'.
...
--
function generic_gsub (new, str, pat, repl, n, cflags, eflags)
  ...
  assert(n == nil or tonumber(n),
            "generic_gsub: 5th argument must be number or nil")

  local reg = new (pat, cflags) -- compile the pattern
  local reps = 0                -- number of replacements made
  local start = 1               -- offset in str for matching operation
  ...
  while not n or n > reps do
    ...
    if start <= to then
      start = to + 1
    elseif start <= #str then -- advance by 1 char (not replaced)
      table.insert(tb_out, string.sub(str, start, start))
      start = start + 1
    else
      break
    end
  ...
  if start <= #str then
    insert(tb_out, sub(str, start))
  end

  return table.concat(tb_out), reps
end
